        ...
        for _, headline in ipairs(org_file.headlines) do
          if headline.level == 2 then
            local item = {
              ...
              scheduled = headline.scheduled and headline.scheduled:is_today(),
              deadline = headline.deadline and headline.deadline:is_today_or_past("day"),
              clocked_in = headline._section:is_clocked_in(),
            }
            if org_data.todo_value == "PLAN" then
